...
def build_income_table(fin_filtered: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if income.empty: return pd.DataFrame(), pd.DataFrame()
    years = (list(income["period"].cat.categories)
             if hasattr(income["period"], "cat") else sorted(income["period"].unique()))
    ...
    for it in spec:
        label, stype = it["label"], it["style"]
        if stype == "header":
            rows.append(pd.Series(name=label, data={y: np.nan for y in years}))
            styles.append(stype); continue
        s = _match_and_sum(income, [k.lower() for k in it["keywords"]])
        row = pd.Series(index=years, dtype=float, name=label)
        row.loc[s.index] = s.values
        # Nếu chưa có Gross Profit thì tự tính = Revenue - COGS
        if label == "Gross Profit" and row.isna().all():
            nr = get_row("Net Revenue"); cogs = get_row("Cost of Goods Sold (COGS)")
            if nr is not None and cogs is not None: row = nr - cogs
